...
class SwiftRadioUITests: XCTestCase {
    ...
    let hamburgerMenu = XCUIApplication().navigationBars["CKUT 90.3 FM"].buttons["icon hamburger"]
    ...
    func assertHamburgerContent() {
        XCTAssertTrue(app.staticTexts["Created by: Matthew Dorfman Source code by: Matthew Fecher"].exists)
    }
    
    func assertAboutContent() {
        XCTAssertTrue(app.buttons["email me"].exists)
        XCTAssertTrue(app.buttons["ckut.ca"].exists)
    }
    ...
    func testMainStationsView() {
        ...
        app.navigationBars["CKUT Radio"].buttons["btn nowPlaying"].tap()
        waitForStationToLoad()
